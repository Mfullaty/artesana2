    ...
    <>
      ...
      `}>
        <div className="p-4 flex justify-between items-center">
          <h2 className={`font-bold text-xl ${isSidebarOpen ? 'block' : 'hidden'}`}>Artesana Admin</h2>
          {!isMobile && (
            <Button
              variant="ghost"
              size="icon"
        ...
        <nav className="mt-4 mx-2 flex-grow">
          {sidebarItems.map((item) => (
            <Link key={item.href} href={item.href}>
              <Button
                className={`w-full justify-start hover:text-primary-foreground  mb-3 ${isSidebarOpen ? 'px-4' : 'px-0 justify-center'} ${pathname === item.href ? "bg-primary" : "bg-transparent text-primary"}`}
                title={item.label}
              >
